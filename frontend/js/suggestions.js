...
function formatRichTextHTML(text, className = 'suggestion-text') {
    ...
    function flushParagraph() {
        ...
        output.push(`<p class="${className}">${formatInlineRichText(paragraph)}</p>`);
        paragraphLines = [];
    }
    ...
    function flushList() {
        ...
        output.push(`<${listTag} class="${className}">${listItems.map(item => `<li>${formatInlineRichText(item)}</li>`).join('')}</${listTag}>`);
        listType = null;
        listItems = [];
    }
    ...
    lines.forEach(rawLine => {
        ...
        if (headingMatch) {
            ...
            const content = formatInlineRichText(headingText);
