...
AUTO_OPEN_FRONTEND = os.getenv("AUTO_OPEN_FRONTEND", "0").strip().lower() in {"1", "true", "yes"}
...
async def auto_open_frontend() -> None:
    """Open the served frontend in a browser for local development runs."""
    if not AUTO_OPEN_FRONTEND:
        return

    # Avoid opening browser on hosted/server environments.
    if os.getenv("RENDER") or os.getenv("CI"):
        return

    if APP_HOST not in {"localhost", "127.0.0.1", "0.0.0.0"}:
        return

    open_host = "localhost" if APP_HOST == "0.0.0.0" else APP_HOST
    url = f"http://{open_host}:{APP_PORT}/"

    try:
        threading.Timer(0.8, lambda: webbrowser.open_new_tab(url)).start()
        logger.info("Auto-opening frontend: %s", url)
    except Exception as exc:
        logger.warning("Could not auto-open frontend: %s", exc)
